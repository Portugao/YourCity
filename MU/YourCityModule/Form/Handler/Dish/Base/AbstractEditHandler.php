 ...
 * It aims on the dish object type.
 */
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'dish';
        $this->objectTypeCapital = 'Dish';
        $this->objectTypeLower = 'dish';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the dish yet as other items are required which must be created before!'));
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of dish area
        $codes[] = 'userIndex';
        // admin index page of dish area
        $codes[] = 'adminIndex';
        // user list of dishes
        $codes[] = 'userView';
        // admin list of dishes
        $codes[] = 'adminView';
        // user list of own dishes
        $codes[] = 'userOwnView';
        // admin list of own dishes
        $codes[] = 'adminOwnView';
        // user detail page of treated dish
        $codes[] = 'userDisplay';
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Dish created.');
                } else {
                    $message = $this->__('Done! Dish updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Dish deleted.');
                break;
            default:
                $message = $this->__('Done! Dish updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUYourCityModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'dish', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
