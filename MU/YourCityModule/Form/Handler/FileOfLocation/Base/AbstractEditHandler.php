...
namespace MU\YourCityModule\Form\Handler\FileOfLocation\Base;

use MU\YourCityModule\Form\Handler\Common\EditHandler;
use MU\YourCityModule\Form\Type\FileOfLocationType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'fileOfLocation';
        $this->objectTypeCapital = 'FileOfLocation';
        $this->objectTypeLower = 'fileoflocation';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the file of location yet as other items are required which must be created before!'));
    ...
    protected function initRelationPresets()
    {
        ...
        if (!empty($this->relationPresets['location'])) {
            ...
            if (null !== $relObj) {
                $relObj->addFilesOfLocation($entity);
            }
        }
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(FileOfLocationType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of file of location area
        $codes[] = 'userIndex';
        // admin index page of file of location area
        $codes[] = 'adminIndex';
        // user list of files of location
        $codes[] = 'userView';
        // admin list of files of location
        $codes[] = 'adminView';
        // user list of own files of location
        $codes[] = 'userOwnView';
        // admin list of own files of location
        $codes[] = 'adminOwnView';
        // user detail page of treated file of location
        $codes[] = 'userDisplay';
        // admin detail page of treated file of location
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        // redirect to the list of filesOfLocation
    ...
    protected function getDefaultMessage($args, $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! File of location created.');
                } else {
                    $message = $this->__('Done! File of location updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! File of location deleted.');
                break;
            default:
                $message = $this->__('Done! File of location updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUYourCityModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'file of location', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
