...
namespace MU\YourCityModule\Form\Handler\ImageOfPart\Base;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'imageOfPart';
        $this->objectTypeCapital = 'ImageOfPart';
        $this->objectTypeLower = 'imageofpart';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the image of part yet as other items are required which must be created before!'));
    ...
    protected function initRelationPresets()
    {
        ...
        $this->relationPresets['part'] = $this->request->get('part', '');
        if (!empty($this->relationPresets['part'])) {
            $relObj = $this->entityFactory->getRepository('part')->selectById($this->relationPresets['part']);
            if (null !== $relObj) {
                $relObj->addImagesOfPart($entity);
            }
        }
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create('MU\YourCityModule\Form\Type\ImageOfPartType', $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of image of part area
        $codes[] = 'userIndex';
        // admin index page of image of part area
        $codes[] = 'adminIndex';
        // user list of images of part
        $codes[] = 'userView';
        // admin list of images of part
        $codes[] = 'adminView';
        // user list of own images of part
        $codes[] = 'userOwnView';
        // admin list of own images of part
        $codes[] = 'adminOwnView';
        // user detail page of treated image of part
        $codes[] = 'userDisplay';
        // admin detail page of treated image of part
        $codes[] = 'adminDisplay';
    
        // user list of parts
        $codes[] = 'userViewParts';
        // admin list of parts
        $codes[] = 'adminViewParts';
        // user list of own parts
        $codes[] = 'userOwnViewParts';
        // admin list of own parts
        $codes[] = 'adminOwnViewParts';
        // user detail page of related part
        $codes[] = 'userDisplayPart';
        // admin detail page of related part
        $codes[] = 'adminDisplayPart';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        // redirect to the list of imagesOfPart
    ...
    protected function getDefaultMessage($args, $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Image of part created.');
                } else {
                    $message = $this->__('Done! Image of part updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Image of part deleted.');
                break;
            default:
                $message = $this->__('Done! Image of part updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUYourCityModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'image of part', 'id' => $entity->createCompositeIdentifier(), 'errorMessage' => $e->getMessage()];
    ...
    protected function getRedirectUrl($args)
    {
        ...
        switch ($this->returnTo) {
            ...
            case 'userViewParts':
            case 'adminViewParts':
                return $this->router->generate('muyourcitymodule_part_' . $routeArea . 'view');
            case 'userOwnViewParts':
            case 'adminOwnViewParts':
                return $this->router->generate('muyourcitymodule_part_' . $routeArea . 'view', [ 'own' => 1 ]);
            case 'userDisplayPart':
            case 'adminDisplayPart':
                if (!empty($this->relationPresets['part'])) {
                    return $this->router->generate('muyourcitymodule_part_' . $routeArea . 'display',  ['id' => $this->relationPresets['part']]);
                }
