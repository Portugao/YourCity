...
namespace MU\YourCityModule\Form\Handler\MenuOfLocation\Base;

use MU\YourCityModule\Form\Handler\Common\EditHandler;
use MU\YourCityModule\Form\Type\MenuOfLocationType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'menuOfLocation';
        $this->objectTypeCapital = 'MenuOfLocation';
        $this->objectTypeLower = 'menuoflocation';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the menu of location yet as other items are required which must be created before!'));
    ...
    protected function initRelationPresets()
    {
        ...
        $this->relationPresets['dishes'] = $this->request->get('dishes', '');
        if (!empty($this->relationPresets['dishes'])) {
            $relObj = $this->entityFactory->getRepository('dish')->selectById($this->relationPresets['dishes']);
            if (null !== $relObj) {
                $relObj->addMenusOfLocation($entity);
            }
        }
        // editable relation, we store the id and assign it now to show it in UI
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(MenuOfLocationType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of menu of location area
        $codes[] = 'userIndex';
        // admin index page of menu of location area
        $codes[] = 'adminIndex';
        ...
        $codes[] = 'adminOwnView';
        // user detail page of treated menu of location
        $codes[] = 'userDisplay';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        // redirect to the list of menusOfLocation
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Menu of location created.');
                } else {
                    $message = $this->__('Done! Menu of location updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Menu of location deleted.');
                break;
            default:
                $message = $this->__('Done! Menu of location updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUYourCityModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'menu of location', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
