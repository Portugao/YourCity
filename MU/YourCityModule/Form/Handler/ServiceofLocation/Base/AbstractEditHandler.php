...
namespace MU\YourCityModule\Form\Handler\ServiceOfLocation\Base;

use MU\YourCityModule\Form\Handler\Common\EditHandler;
use MU\YourCityModule\Form\Type\ServiceOfLocationType;
 ...
 * It aims on the service of location object type.
 */
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'serviceOfLocation';
        $this->objectTypeCapital = 'ServiceOfLocation';
        $this->objectTypeLower = 'serviceoflocation';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the service of location yet as other items are required which must be created before!'));
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(ServiceOfLocationType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of service of location area
        $codes[] = 'userIndex';
        // admin index page of service of location area
        $codes[] = 'adminIndex';
        // user list of services of location
        $codes[] = 'userView';
        // admin list of services of location
        $codes[] = 'adminView';
        // user list of own services of location
        $codes[] = 'userOwnView';
        // admin list of own services of location
        $codes[] = 'adminOwnView';
        // user detail page of treated service of location
        $codes[] = 'userDisplay';
        // admin detail page of treated service of location
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        // redirect to the list of servicesOfLocation
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Service of location created.');
                } else {
                    $message = $this->__('Done! Service of location updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Service of location deleted.');
                break;
            default:
                $message = $this->__('Done! Service of location updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUYourCityModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'service of location', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
    ...
    protected function getRedirectUrl($args)
    {
        ...
        if ($this->request->getSession()->has('muyourcitymodule' . $this->objectTypeCapital . 'Referer')) {
            $this->request->getSession()->remove('muyourcitymodule' . $this->objectTypeCapital . 'Referer');
        }
