...
namespace MU\YourCityModule\Form\Handler\ImageOfBranch\Base;
 ...
 * It aims on the image of branch object type.
 */
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'imageOfBranch';
        $this->objectTypeCapital = 'ImageOfBranch';
        $this->objectTypeLower = 'imageofbranch';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the image of branch yet as other items are required which must be created before!'));
    ...
    protected function initRelationPresets()
    {
        ...
        $this->relationPresets['branch'] = $this->request->get('branch', '');
        if (!empty($this->relationPresets['branch'])) {
            $relObj = $this->entityFactory->getRepository('branch')->selectById($this->relationPresets['branch']);
            if (null !== $relObj) {
                $relObj->addImagesOfBranch($entity);
            }
        }
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create('MU\YourCityModule\Form\Type\ImageOfBranchType', $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of image of branch area
        $codes[] = 'userIndex';
        // admin index page of image of branch area
        $codes[] = 'adminIndex';
        // user list of images of branch
        $codes[] = 'userView';
        // admin list of images of branch
        $codes[] = 'adminView';
        // user list of own images of branch
        $codes[] = 'userOwnView';
        // admin list of own images of branch
        $codes[] = 'adminOwnView';
        // user detail page of treated image of branch
        $codes[] = 'userDisplay';
        // admin detail page of treated image of branch
        $codes[] = 'adminDisplay';
    
        // user list of branches
        $codes[] = 'userViewBranches';
        // admin list of branches
        $codes[] = 'adminViewBranches';
        // user list of own branches
        $codes[] = 'userOwnViewBranches';
        // admin list of own branches
        $codes[] = 'adminOwnViewBranches';
        // user detail page of related branch
        $codes[] = 'userDisplayBranch';
        // admin detail page of related branch
        $codes[] = 'adminDisplayBranch';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        // redirect to the list of imagesOfBranch
    ...
    protected function getDefaultMessage($args, $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Image of branch created.');
                } else {
                    $message = $this->__('Done! Image of branch updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Image of branch deleted.');
                break;
            default:
                $message = $this->__('Done! Image of branch updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUYourCityModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'image of branch', 'id' => $entity->createCompositeIdentifier(), 'errorMessage' => $e->getMessage()];
    ...
    protected function getRedirectUrl($args)
    {
        ...
        switch ($this->returnTo) {
            ...
            case 'adminDisplay':
                ...
                return $this->getDefaultReturnUrl($args);
            case 'userViewBranches':
            case 'adminViewBranches':
                return $this->router->generate('muyourcitymodule_branch_' . $routeArea . 'view');
            case 'userOwnViewBranches':
            case 'adminOwnViewBranches':
                return $this->router->generate('muyourcitymodule_branch_' . $routeArea . 'view', [ 'own' => 1 ]);
            case 'userDisplayBranch':
            case 'adminDisplayBranch':
                if (!empty($this->relationPresets['branch'])) {
                    return $this->router->generate('muyourcitymodule_branch_' . $routeArea . 'display',  ['id' => $this->relationPresets['branch']]);
                }
