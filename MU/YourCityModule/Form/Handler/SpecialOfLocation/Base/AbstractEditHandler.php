...
namespace MU\YourCityModule\Form\Handler\SpecialOfLocation\Base;

use MU\YourCityModule\Form\Handler\Common\EditHandler;
use MU\YourCityModule\Form\Type\SpecialOfLocationType;
...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'specialOfLocation';
        $this->objectTypeCapital = 'SpecialOfLocation';
        $this->objectTypeLower = 'specialoflocation';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the special of location yet as other items are required which must be created before!'));
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(SpecialOfLocationType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of special of location area
        $codes[] = 'userIndex';
        // admin index page of special of location area
        $codes[] = 'adminIndex';
        // user list of specials of location
        $codes[] = 'userView';
        // admin list of specials of location
        $codes[] = 'adminView';
        // user list of own specials of location
        $codes[] = 'userOwnView';
        // admin list of own specials of location
        $codes[] = 'adminOwnView';
        // user detail page of treated special of location
        $codes[] = 'userDisplay';
        // admin detail page of treated special of location
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultReturnUrl($args)
    {
        ...
        // redirect to the list of specialsOfLocation
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Special of location created.');
                } else {
                    $message = $this->__('Done! Special of location updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Special of location deleted.');
                break;
            default:
                $message = $this->__('Done! Special of location updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUYourCityModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'special of location', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
