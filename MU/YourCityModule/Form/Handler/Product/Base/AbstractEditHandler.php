...
abstract class AbstractEditHandler extends EditHandler
{
    ...
    public function processForm(array $templateParameters)
    {
        ...
        $this->objectTypeCapital = 'Product';
        $this->objectTypeLower = 'product';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the product yet as other items are required which must be created before!'));
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of product area
        $codes[] = 'userIndex';
        // admin index page of product area
        $codes[] = 'adminIndex';
        // user list of products
        $codes[] = 'userView';
        // admin list of products
        $codes[] = 'adminView';
        // user list of own products
        $codes[] = 'userOwnView';
        // admin list of own products
        $codes[] = 'adminOwnView';
        // user detail page of treated product
        $codes[] = 'userDisplay';
        // admin detail page of treated product
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Product created.');
                } else {
                    $message = $this->__('Done! Product updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Product deleted.');
                break;
            default:
                $message = $this->__('Done! Product updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUYourCityModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'product', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
