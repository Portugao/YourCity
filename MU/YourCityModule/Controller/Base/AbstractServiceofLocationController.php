...
abstract class AbstractServiceOfLocationController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('muyourcitymodule_serviceoflocation_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'muyourcitymodule_serviceoflocation_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
    ...
    protected function displayInternal(Request $request, ServiceOfLocationEntity $serviceOfLocation, $isAdmin = false)
    {
        ...
        $instanceId = $serviceOfLocation->getKey();
        ...
        if ('ics' == $request->getRequestFormat()) {
            $fileName = $objectType . '_' .
                (property_exists($serviceOfLocation, 'slug')
                    ? $serviceOfLocation['slug']
                    : $this->get('mu_yourcity_module.entity_display_helper')->getFormattedTitle($serviceOfLocation)
                ) . '.ics'
            ;
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_yourcity_module.form.handler.serviceoflocation');
    ...
    protected function deleteInternal(Request $request, ServiceOfLocationEntity $serviceOfLocation, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'MUYourCityModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'service of location', 'id' => $serviceOfLocation->getKey()];
        ...
        // redirect to the list of services of location
        $redirectRoute = 'muyourcitymodule_serviceoflocation_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        $form = $this->createForm('Zikula\Bundle\FormExtensionBundle\Form\Type\DeletionType', $serviceOfLocation);
        $hookHelper = $this->get('mu_yourcity_module.hook_helper');
        
        // Call form aware display hooks
        $formHook = $hookHelper->callFormDisplayHooks($form, $serviceOfLocation, FormAwareCategory::TYPE_DELETE);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                // Let any ui hooks perform additional validation actions
                $validationErrors = $hookHelper->callValidationHooks($serviceOfLocation, UiHooksCategory::TYPE_VALIDATE_DELETE);
                if (count($validationErrors) > 0) {
                    ...
                    $hookHelper->callFormProcessHooks($form, $serviceOfLocation, FormAwareCategory::TYPE_PROCESS_DELETE);
                    
                    // Let any ui hooks know that we have deleted the service of location
                    $hookHelper->callProcessHooks($serviceOfLocation, UiHooksCategory::TYPE_PROCESS_DELETE);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'action' => $action, 'entity' => 'service of location', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('muyourcitymodule_serviceoflocation_display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('muyourcitymodule_serviceoflocation_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
