...
abstract class AbstractProductController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('muyourcitymodule_product_' . $templateParameters['routeArea'] . 'view');
    }
    /**
     ...
     *
     * @param Request $request Current request instance
     * @param ProductEntity $product Treated product instance
     *
     * @return Response Output
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown by param converter if product to be displayed isn't found
     */
    public function adminDisplayAction(Request $request, ProductEntity $product)
    {
        return $this->displayInternal($request, $product, true);
    }
     ...
     * @throws NotFoundHttpException Thrown by param converter if product to be displayed isn't found
     */
    public function displayAction(Request $request, ProductEntity $product)
    {
        return $this->displayInternal($request, $product, false);
    }
    
    /**
     * This method includes the common implementation code for adminDisplay() and display().
     */
    protected function displayInternal(Request $request, ProductEntity $product, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'product';
        $permLevel = $isAdmin ? ACCESS_ADMIN : ACCESS_READ;
     ...
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown by form handler if product to be edited isn't found
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_yourcity_module.form.handler.product');
     ...
     *
     * @throws AccessDeniedException Thrown if the user doesn't have required permissions
     * @throws NotFoundHttpException Thrown by param converter if product to be deleted isn't found
     * @throws RuntimeException      Thrown if another critical error occurs (e.g. workflow actions not available)
     */
    public function adminDeleteAction(Request $request, ProductEntity $product)
    {
        return $this->deleteInternal($request, $product, true);
    }
     ...
     * @throws RuntimeException      Thrown if another critical error occurs (e.g. workflow actions not available)
     */
    public function deleteAction(Request $request, ProductEntity $product)
    {
        return $this->deleteInternal($request, $product, false);
    }
    
    /**
     * This method includes the common implementation code for adminDelete() and delete().
     */
    protected function deleteInternal(Request $request, ProductEntity $product, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'product';
        ...
        $actions = $workflowHelper->getActionsForObject($product);
        ...
        $redirectRoute = 'muyourcitymodule_product_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        if (!$deleteAllowed) {
            $this->addFlash('error', $this->__('Error! It is not allowed to delete this product.'));
            $logger->error('{app}: User {user} tried to delete the {entity} with id {id}, but this action was not allowed.', $logArgs);
        
            return $this->redirectToRoute($redirectRoute);
        }
        
        $form = $this->createForm('Zikula\Bundle\FormExtensionBundle\Form\Type\DeletionType', $product);
        $hookHelper = $this->get('mu_yourcity_module.hook_helper');
        
        // Call form aware display hooks
        $formHook = $hookHelper->callFormDisplayHooks($form, $product, FormAwareCategory::TYPE_DELETE);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                // Let any ui hooks perform additional validation actions
                $validationErrors = $hookHelper->callValidationHooks($product, UiHooksCategory::TYPE_VALIDATE_DELETE);
                if (count($validationErrors) > 0) {
                    ...
                    $success = $workflowHelper->executeAction($product, $deleteActionId);
                    if ($success) {
                    ...
                    $hookHelper->callFormProcessHooks($form, $product, FormAwareCategory::TYPE_PROCESS_DELETE);
                    
                    // Let any ui hooks know that we have deleted the product
                    $hookHelper->callProcessHooks($product, UiHooksCategory::TYPE_PROCESS_DELETE);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'action' => $action, 'entity' => 'product', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'entity' => 'product', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'action' => $action, 'entity' => 'product', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('muyourcitymodule_product_display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('muyourcitymodule_product_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
