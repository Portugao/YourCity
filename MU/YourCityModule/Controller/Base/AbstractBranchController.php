...
/**
 * Branch controller base class.
 */
abstract class AbstractBranchController extends AbstractController
{
    /**
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('muyourcitymodule_branch_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'muyourcitymodule_branch_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
     ...
     * @ParamConverter("branch", class="MUYourCityModule:BranchEntity", options = {"repository_method" = "selectById", "mapping": {"id": "id"}, "map_method_signature" = true})
     * @Cache(lastModified="branch.getUpdatedDate()", ETag="'Branch' ~ branch.getid() ~ branch.getUpdatedDate().format('U')")
     *
     * @param Request $request Current request instance
    ...
    {
        return $this->displayInternal($request, $branch, true);
    }
    ...
    {
        return $this->displayInternal($request, $branch, false);
    }
    
    /**
     * This method includes the common implementation code for adminDisplay() and display().
     */
    protected function displayInternal(Request $request, BranchEntity $branch, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'branch';
        $permLevel = $isAdmin ? ACCESS_ADMIN : ACCESS_READ;
        ...
        if ('ics' == $request->getRequestFormat()) {
            $fileName = $objectType . '_' .
                (property_exists($branch, 'slug')
                    ? $branch['slug']
                    : $this->get('mu_yourcity_module.entity_display_helper')->getFormattedTitle($branch)
                ) . '.ics'
            ;
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_yourcity_module.form.handler.branch');
    ...
    {
        return $this->deleteInternal($request, $branch, true);
    }
    ...
    {
        return $this->deleteInternal($request, $branch, false);
    }
    
    /**
     * This method includes the common implementation code for adminDelete() and delete().
     */
    protected function deleteInternal(Request $request, BranchEntity $branch, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'branch';
        ...
        $logArgs = ['app' => 'MUYourCityModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'branch', 'id' => $branch->getKey()];
        
        // determine available workflow actions
        $workflowHelper = $this->get('mu_yourcity_module.workflow_helper');
        $actions = $workflowHelper->getActionsForObject($branch);
        ...
        // redirect to the list of branches
        $redirectRoute = 'muyourcitymodule_branch_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        if (!$deleteAllowed) {
            $this->addFlash('error', $this->__('Error! It is not allowed to delete this branch.'));
            $logger->error('{app}: User {user} tried to delete the {entity} with id {id}, but this action was not allowed.', $logArgs);
        
            return $this->redirectToRoute($redirectRoute);
        }
        
        $form = $this->createForm('Zikula\Bundle\FormExtensionBundle\Form\Type\DeletionType', $branch);
        $hookHelper = $this->get('mu_yourcity_module.hook_helper');
        
        // Call form aware display hooks
        $formHook = $hookHelper->callFormDisplayHooks($form, $branch, FormAwareCategory::TYPE_DELETE);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                // Let any ui hooks perform additional validation actions
                $validationErrors = $hookHelper->callValidationHooks($branch, UiHooksCategory::TYPE_VALIDATE_DELETE);
                if (count($validationErrors) > 0) {
                    ...
                    $success = $workflowHelper->executeAction($branch, $deleteActionId);
                    if ($success) {
                    ...
                    $hookHelper->callFormProcessHooks($form, $branch, FormAwareCategory::TYPE_PROCESS_DELETE);
                    
                    // Let any ui hooks know that we have deleted the branch
                    $hookHelper->callProcessHooks($branch, UiHooksCategory::TYPE_PROCESS_DELETE);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'action' => $action, 'entity' => 'branch', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'entity' => 'branch', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'action' => $action, 'entity' => 'branch', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('muyourcitymodule_branch_display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('muyourcitymodule_branch_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
