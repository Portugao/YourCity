 ...
 */
abstract class AbstractPartOfMenuController extends AbstractController
{
    /**
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('muyourcitymodule_partofmenu_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    {
        return $this->displayInternal($request, $partOfMenu, true);
    }
    ...
    {
        return $this->displayInternal($request, $partOfMenu, false);
    }
    
    /**
     * This method includes the common implementation code for adminDisplay() and display().
     */
    protected function displayInternal(Request $request, PartOfMenuEntity $partOfMenu, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'partOfMenu';
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_yourcity_module.form.handler.partofmenu');
    ...
    {
        return $this->deleteInternal($request, $partOfMenu, true);
    }
    ...
    {
        return $this->deleteInternal($request, $partOfMenu, false);
    }
    
    /**
     * This method includes the common implementation code for adminDelete() and delete().
     */
    protected function deleteInternal(Request $request, PartOfMenuEntity $partOfMenu, $isAdmin = false)
    {
        // parameter specifying which type of objects we are treating
        $objectType = 'partOfMenu';
        ...
        // redirect to the list of parts of menu
        $redirectRoute = 'muyourcitymodule_partofmenu_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        if (!$deleteAllowed) {
            $this->addFlash('error', $this->__('Error! It is not allowed to delete this part of menu.'));
        ...
        $formHook = $hookHelper->callFormDisplayHooks($form, $partOfMenu, FormAwareCategory::TYPE_DELETE);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                // Let any ui hooks perform additional validation actions
                $validationErrors = $hookHelper->callValidationHooks($partOfMenu, UiHooksCategory::TYPE_VALIDATE_DELETE);
                if (count($validationErrors) > 0) {
                    ...
                    $success = $workflowHelper->executeAction($partOfMenu, $deleteActionId);
                    if ($success) {
                    ...
                    $hookHelper->callFormProcessHooks($form, $partOfMenu, FormAwareCategory::TYPE_PROCESS_DELETE);
                    
                    // Let any ui hooks know that we have deleted the part of menu
                    $hookHelper->callProcessHooks($partOfMenu, UiHooksCategory::TYPE_PROCESS_DELETE);
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'muyourcitymodule_partofmenu_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'action' => $action, 'entity' => 'part of menu', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'entity' => 'part of menu', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'action' => $action, 'entity' => 'part of menu', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('muyourcitymodule_partofmenu_display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('muyourcitymodule_partofmenu_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
