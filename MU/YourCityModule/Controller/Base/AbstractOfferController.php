...
abstract class AbstractOfferController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('muyourcitymodule_offer_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'muyourcitymodule_offer_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
    ...
    protected function displayInternal(Request $request, OfferEntity $offer, $isAdmin = false)
    {
        ...
        if ('ics' == $request->getRequestFormat()) {
            $fileName = $objectType . '_' .
                (property_exists($offer, 'slug')
                    ? $offer['slug']
                    : $this->get('mu_yourcity_module.entity_display_helper')->getFormattedTitle($offer)
                ) . '.ics'
            ;
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_yourcity_module.form.handler.offer');
    ...
    protected function deleteInternal(Request $request, OfferEntity $offer, $isAdmin = false)
    {
        ...
        $redirectRoute = 'muyourcitymodule_offer_' . ($isAdmin ? 'admin' : '') . 'view';
        ...
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                ...
                $validationHooksPassed = $hookHelper->callValidationHooks($offer, 'validate_delete');
                if ($validationHooksPassed) {
                    ...
                    // Let any hooks know that we have deleted the offer
                    $hookHelper->callProcessHooks($offer, 'process_delete', null);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'action' => $action, 'entity' => 'offer', 'id' => $itemId, 'errorMessage' => $e->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'entity' => 'offer', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'action' => $action, 'entity' => 'offer', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('muyourcitymodule_offer_' . /*($isAdmin ? 'admin' : '') . */'display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('muyourcitymodule_offer_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
