...
abstract class AbstractAbonnementController extends AbstractController
{
    ...
    protected function deleteInternal(Request $request, AbonnementEntity $abonnement, $isAdmin = false)
    {
        ...
        $formHook = $hookHelper->callFormDisplayHooks($form, $abonnement, FormAwareCategory::TYPE_DELETE);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                // Let any ui hooks perform additional validation actions
                $validationErrors = $hookHelper->callValidationHooks($abonnement, UiHooksCategory::TYPE_VALIDATE_DELETE);
                if (count($validationErrors) > 0) {
                    ...
                    $hookHelper->callFormProcessHooks($form, $abonnement, FormAwareCategory::TYPE_PROCESS_DELETE);
                    
                    // Let any ui hooks know that we have deleted the abonnement
                    $hookHelper->callProcessHooks($abonnement, UiHooksCategory::TYPE_PROCESS_DELETE);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUYourCityModule', 'user' => $userName, 'action' => $action, 'entity' => 'abonnement', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('muyourcitymodule_abonnement_display', $urlArgs);
            }
