 ...
 *
 * This is the base repository class for image of location entities.
 */
abstract class AbstractImageOfLocationRepository extends EntityRepository
{
    /**
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('MU\YourCityModule\Entity\ImageOfLocationEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'MUYourCityModule', 'entity' => 'image of location'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApiInterface $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MU\YourCityModule\Entity\ImageOfLocationEntity', 'tbl')
        ...
        $logArgs = ['app' => 'MUYourCityModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'images of location', 'userid' => $userId];
     ...
     *
     * @return array|imageOfLocationEntity Retrieved data array or imageOfLocationEntity instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection Collection containing retrieved imageOfLocationEntity instances
     */
    ...
    public function getListQueryBuilder($where = '', $orderBy = '', $useJoins = true, $slimMode = false)
    {
        ...
        if ((!$useJoins || !$slimMode) && null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addCommonViewFilters('imageOfLocation', $qb);
        }
    ...
    public function selectSearch($fragment = '', $exclude = [], $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addSearchFilter('imageOfLocation', $qb, $fragment);
        }
    ...
    {
        $selection = 'COUNT(tbl.id) AS numImagesOfLocation';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MU\YourCityModule\Entity\ImageOfLocationEntity', 'tbl');
    ...
    public function selectCount($where = '', $useJoins = false, $parameters = [])
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('imageOfLocation', $qb, $parameters);
        }
     ...
     * @param integer $excludeId  Id of images of location to exclude (optional)
     *
     * @return boolean Result of this check, true if the given image of location does not already exist
     */
