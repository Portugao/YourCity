 ...
 *
 * This is the base repository class for branch entities.
 */
abstract class AbstractBranchRepository extends EntityRepository
{
    /**
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('MU\YourCityModule\Entity\BranchEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'MUYourCityModule', 'entity' => 'branch'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApiInterface $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MU\YourCityModule\Entity\BranchEntity', 'tbl')
        ...
        $logArgs = ['app' => 'MUYourCityModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'branches', 'userid' => $userId];
     ...
     *
     * @return array|branchEntity Retrieved data array or branchEntity instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection Collection containing retrieved branchEntity instances
     */
    ...
    public function getListQueryBuilder($where = '', $orderBy = '', $useJoins = true, $slimMode = false)
    {
        ...
        if ((!$useJoins || !$slimMode) && null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addCommonViewFilters('branch', $qb);
        }
    ...
    {
        $selection = 'COUNT(tbl.id) AS numBranches';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MU\YourCityModule\Entity\BranchEntity', 'tbl');
    ...
    public function selectCount($where = '', $useJoins = false, $parameters = [])
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('branch', $qb, $parameters);
        }
     ...
     * @param integer $excludeId  Id of branches to exclude (optional)
     *
     * @return boolean Result of this check, true if the given branch does not already exist
     */
