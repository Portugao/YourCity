 ...
 *
 * This is the base repository class for product entities.
 */
abstract class AbstractProductRepository extends EntityRepository
{
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('MU\YourCityModule\Entity\ProductEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'MUYourCityModule', 'entity' => 'product'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApiInterface $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MU\YourCityModule\Entity\ProductEntity', 'tbl')
        ...
        $logArgs = ['app' => 'MUYourCityModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'products', 'userid' => $userId];
     ...
     *
     * @return array|productEntity Retrieved data array or productEntity instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection Collection containing retrieved productEntity instances
     */
    ...
    public function getListQueryBuilder($where = '', $orderBy = '', $useJoins = true, $slimMode = false)
    {
        ...
        if ((!$useJoins || !$slimMode) && null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addCommonViewFilters('product', $qb);
        }
    ...
    public function selectSearch($fragment = '', $exclude = [], $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addSearchFilter('product', $qb, $fragment);
        }
    ...
    {
        $selection = 'COUNT(tbl.id) AS numProducts';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MU\YourCityModule\Entity\ProductEntity', 'tbl');
    ...
    public function selectCount($where = '', $useJoins = false, $parameters = [])
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('product', $qb, $parameters);
        }
     ...
     * @param integer $excludeId  Id of products to exclude (optional)
     *
     * @return boolean Result of this check, true if the given product does not already exist
     */
