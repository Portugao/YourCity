 ...
 *
 * This is the base repository class for abonnement entities.
 */
abstract class AbstractAbonnementRepository extends EntityRepository
{
    /**
    ...
    public function getAllowedSortingFields()
    {
        return [
            ...
            'sendMessageForMenus',
            'showOffers',
            'sendMessageForOffers',
            'showEvents',
            'sendMessageForEvents',
            'showProducts',
            'sendMessageForProducts',
            'createdBy',
            'createdDate',
            'updatedBy',
            'updatedDate',
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('MU\YourCityModule\Entity\AbonnementEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'MUYourCityModule', 'entity' => 'abonnement'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApiInterface $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MU\YourCityModule\Entity\AbonnementEntity', 'tbl')
        ...
        $logArgs = ['app' => 'MUYourCityModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'abonnements', 'userid' => $userId];
     ...
     *
     * @return array|abonnementEntity Retrieved data array or abonnementEntity instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection Collection containing retrieved abonnementEntity instances
     */
    ...
    public function getListQueryBuilder($where = '', $orderBy = '', $useJoins = true, $slimMode = false)
    {
        ...
        if ((!$useJoins || !$slimMode) && null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addCommonViewFilters('abonnement', $qb);
        }
    ...
    public function selectSearch($fragment = '', $exclude = [], $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true)
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->addSearchFilter('abonnement', $qb, $fragment);
        }
    ...
    {
        $selection = 'COUNT(tbl.id) AS numAbonnements';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MU\YourCityModule\Entity\AbonnementEntity', 'tbl');
    ...
    public function selectCount($where = '', $useJoins = false, $parameters = [])
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('abonnement', $qb, $parameters);
        }
     ...
     * @param integer $excludeId  Id of abonnements to exclude (optional)
     *
     * @return boolean Result of this check, true if the given abonnement does not already exist
     */
