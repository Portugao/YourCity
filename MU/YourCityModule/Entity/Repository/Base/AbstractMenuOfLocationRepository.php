 ...
 *
 * This is the base repository class for menu of location entities.
 */
abstract class AbstractMenuOfLocationRepository extends EntityRepository
{
    /**
    ...
    public function truncateTable(LoggerInterface $logger)
    {
        ...
        $qb->delete('MU\YourCityModule\Entity\MenuOfLocationEntity', 'tbl');
        $query = $qb->getQuery();
    
        $query->execute();
    
        $logArgs = ['app' => 'MUYourCityModule', 'entity' => 'menu of location'];
    ...
    public function updateCreator($userId, $newUserId, TranslatorInterface $translator, LoggerInterface $logger, CurrentUserApi $currentUserApi)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MU\YourCityModule\Entity\MenuOfLocationEntity', 'tbl')
        ...
        $logArgs = ['app' => 'MUYourCityModule', 'user' => $currentUserApi->get('uname'), 'entities' => 'menus of location', 'userid' => $userId];
     ...
     *
     * @return array|menuOfLocationEntity Retrieved data array or menuOfLocationEntity instance
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false)
     *
     * @return ArrayCollection Collection containing retrieved menuOfLocationEntity instances
     */
    ...
    {
        $selection = 'COUNT(tbl.id) AS numMenusOfLocation';
        if (true === $useJoins) {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('MU\YourCityModule\Entity\MenuOfLocationEntity', 'tbl');
    ...
    public function selectCount($where = '', $useJoins = false, $parameters = [])
    {
        ...
        if (null !== $this->collectionFilterHelper) {
            $qb = $this->collectionFilterHelper->applyDefaultFilters('menuOfLocation', $qb, $parameters);
        }
     ...
     * @param integer $excludeId  Id of menus of location to exclude (optional)
     *
     * @return boolean Result of this check, true if the given menu of location does not already exist
     */
    ...
    {
        $selection = ', tblLocation, tblDishes, tblPartsOfMenu';
