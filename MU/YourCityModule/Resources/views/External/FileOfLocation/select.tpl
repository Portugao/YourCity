...
{assign var='baseID' value='fileOfLocation'}
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='File of location'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='fileOfLocation' from=$items}
                        <option value="{$fileOfLocation->getKey()}"{if $selectedId eq $fileOfLocation->getKey()} selected="selected"{/if}>{$fileOfLocation->getName()}</option>
                    {foreachelse}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Sort" name="sort" class="form-control">
                    ...
                    <option value="fileOfFile"{if $sort eq 'fileOfFile'} selected="selected"{/if}>{gt text='File of file'}</option>
    ...
    <div class="col-sm-4">
        <div id="{$baseID}Preview" style="border: 1px dotted #a3a3a3; padding: .2em .5em">
            <p><strong>{gt text='File of location information'}</strong></p>
