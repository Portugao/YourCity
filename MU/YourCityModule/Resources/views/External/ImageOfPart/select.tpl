...
{assign var='baseID' value='imageOfPart'}
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Image of part'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='imageOfPart' from=$items}
                        <option value="{$imageOfPart.id}"{if $selectedId eq $imageOfPart.id} selected="selected"{/if}>{$imageOfPart->getTitleFromDisplayPattern()}</option>
                    {foreachelse}
    ...
    <div class="col-sm-4">
        <div id="{$baseID}Preview" style="border: 1px dotted #a3a3a3; padding: .2em .5em">
            <p><strong>{gt text='Image of part information'}</strong></p>
