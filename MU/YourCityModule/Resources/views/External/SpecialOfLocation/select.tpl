...
{assign var='baseID' value='specialOfLocation'}
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Special of location'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='specialOfLocation' from=$items}
                        <option value="{$specialOfLocation.id}"{if $selectedId eq $specialOfLocation.id} selected="selected"{/if}>{$specialOfLocation->getName()}</option>
                    {foreachelse}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Sort" name="sort" class="form-control">
                    ...
                    <option value="iconForSpecial"{if $sort eq 'iconForSpecial'} selected="selected"{/if}>{gt text='Icon for special'}</option>
    ...
    <div class="col-sm-4">
        <div id="{$baseID}Preview" style="border: 1px dotted #a3a3a3; padding: .2em .5em">
            <p><strong>{gt text='Special of location information'}</strong></p>
