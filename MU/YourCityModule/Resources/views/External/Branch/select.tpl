...
{assign var='baseID' value='branch'}
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Branch'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='branch' from=$items}
                        <option value="{$branch.id}"{if $selectedId eq $branch.id} selected="selected"{/if}>{$branch->getName()}</option>
                    {foreachelse}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Sort" name="sort" class="form-control">
                    ...
                    <option value="imageofBranch"{if $sort eq 'imageofBranch'} selected="selected"{/if}>{gt text='Imageof branch'}</option>
    ...
    <div class="col-sm-4">
        <div id="{$baseID}Preview" style="border: 1px dotted #a3a3a3; padding: .2em .5em">
            <p><strong>{gt text='Branch information'}</strong></p>
