...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
    ...
    public function contentGetTypes(\Zikula_Event $event)
    {
        ...
        $types->add('MUYourCityModule_ContentType_Item');
        
        // plugin for showing a list of multiple items
        $types->add('MUYourCityModule_ContentType_ItemList');
    }
     ...
     * MUYourCityModule will use this to add a javascript helper to add custom items.
     *
     * You can access general data available in the event.
     *
     * The event name:
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        $targetDir = 'web/modules/muyourcity';
        $finder = new Finder();
        if (!$this->filesystem->exists($targetDir)) {
            $this->filesystem->mkdir($targetDir, 0777);
            if (is_dir($originDir = 'modules/MU/YourCityModule/Resources/public')) {
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
            if (is_dir($originDir = 'modules/MU/YourCityModule/Resources/scribite')) {
                $targetDir .= '/scribite';
                $this->filesystem->mkdir($targetDir, 0777);
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
        ...
        $event->getHelperCollection()->add(
            [
                ...
                'type' => 'javascript',
                'path' => $this->request->getBasePath() . '/web/modules/muyourcity/js/MUYourCityModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            ...
            'path' => $this->request->getBasePath() . '/web/modules/muyourcity/scribite/CKEditor/muyourcitymodule/',
            'file' => 'plugin.js',
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'muyourcitymodule',
            'path' => $this->request->getBasePath() . '/web/modules/muyourcity/scribite/Quill/muyourcitymodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'muyourcitymodule',
            'path' => $this->request->getBasePath() . '/web/modules/muyourcity/scribite/Summernote/muyourcitymodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'muyourcitymodule',
            'path' => $this->request->getBasePath() . '/web/modules/muyourcity/scribite/TinyMce/muyourcitymodule/plugin.js'
        ]);
    }
}
